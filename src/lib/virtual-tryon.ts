/**
 * Virtual Try-On Engine v30 — CONCRETE & ACCURATE (no more mismatches)
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  WHY v30?  (ROOT CAUSE FIX for "different person AND different product")
 *  ─────────────────────────────────────────────────────────────────────────
 *  v28/v29 still produced mismatches for sarees and jewelry because:
 *
 *  1. SAREE product images show a BLACK MANNEQUIN wearing the saree + extra
 *     jewelry items on a BROWN background. The image composite's bg-removal
 *     only removed the brown bg — the BLACK MANNEQUIN stayed. So the
 *     composite placed a black mannequin (wearing the saree) OVER the user's
 *     face → user saw "a different person" (the mannequin).
 *
 *  2. POLLINATIONS (used as saree fallback) generates a completely NEW person
 *     from text — it CANNOT preserve the user's face. So the user saw
 *     "a different person AND a different product" (both generated by AI).
 *
 *  v30 FIXES this PERMANENTLY:
 *
 *    • For SAREES (full-body Indian garments on mannequins):
 *      PRIMARY = Showcase Composite (split-view — always shows user's real
 *        face + real product side-by-side). 100% reliable, no mismatch.
 *      Gemini (if valid key set) → true image editing.
 ...
 *      PRIMARY = Image Composite (sharp) — but ONLY if mannequin detection
 *        passes. For jewelry on BLACK backgrounds (common), the bg-removal
 *        eliminates BOTH the black bg AND the black mannequin, leaving just
 *        the jewelry piece → composite works perfectly.
 *      FALLBACK = Showcase Composite (if mannequin detected or composite fails).
 *      Gemini (if valid key set).
 *
 *    • For GARMENTS (shirts, dresses, fashion, kids):
 *      PRIMARY = IDM-VTON HF Space (real VTON — preserves face + garment).
 *        IDM-VTON handles upper-body Western garments well.
 *      FALLBACK = Showcase Composite (garment product images have models →
 *        composite would show model over face).
 *      Gemini (if valid key set).
 *
 *  STRATEGY ORDER (v30):
 *
 ...
 *         - JEWELRY/WATCHES/ACCESSORIES: Image Composite (with mannequin check)
 *         - SAREES: Showcase Composite (no composite, no Pollinations)
 *      3. Showcase Composite (ULTIMATE FALLBACK — 100% reliable)
 *
 *  On LOCAL (sandbox):
 *    1. ZAI image-edit (PRIMARY — handles ALL categories)
 *    2. Image Composite (with mannequin check)
 *    3. IDM-VTON (garments only)
 *    4. Showcase Composite (ultimate fallback)
 *
 *  100% FREE FOREVER: ZAI (free in sandbox), IDM-VTON (free HF Space),
 *  Image Composite (sharp — free, instant), Showcase Composite (sharp —
 *  free, instant), Gemini (free tier 1500/day if key set). No paid APIs.
 * ─────────────────────────────────────────────────────────────────────────
 */
...
export async function isTryOnServiceReady(): Promise<{
  ...
  return {
    ready: true,
    engine: isVercel ? 'showcase-composite-idm-vton-v30' : 'zai-image-edit',
    reason: isVercel
      ? 'v30: Showcase Composite (100% reliable for sarees — no mismatch), Image Composite (mannequin-checked) for jewelry, IDM-VTON for garments. Free forever, no auth required.'
...
export async function performVirtualTryOn(input: TryOnInput): Promise<TryOnResult> {
  ...
  console.log(`[virtual-tryon] v30 start: "${input.productName}" (${input.categorySlug}) — VERCEL=${isVercel}, hasGeminiKey=${hasGeminiKey}, hasSelfie=${!!input.selfieData}, hasProductImg=${!!input.productImageBase64}`)
  ...
  // ═══════════════════════════════════════════════════════════════════
  //  v30 STRATEGY ORDER — CONCRETE & ACCURATE (no more mismatches)
  //
  ...
  //
  //  v30 FIX:
  //  - SAREES: Skip composite AND Pollinations. Use Showcase Composite
  //    (split-view) as PRIMARY → always shows real face + real product.
  //  - JEWELRY/WATCHES/ACCESSORIES: Image Composite is PRIMARY, but with
  //    NEW mannequin detection. If mannequin is detected, skip to showcase.
  ...
  //  - GARMENTS: IDM-VTON is PRIMARY. Showcase as fallback (skip composite
  //    since garment images have models → composite would show model over face).
  //
  //  On VERCEL (production):
  //    ALL categories:
  //      1. Gemini Nano Banana (if valid AIzaSy... key set) — TRUE image
  ...
  //         - SAREES: Showcase Composite (skip composite, skip Pollinations)
  //      3. ★ SHOWCASE COMPOSITE (ULTIMATE FALLBACK — 100% reliable) ★
  //
  //  On LOCAL (sandbox):
  //    1. ZAI image-edit (PRIMARY — handles ALL categories)
  ...
  console.log(`[virtual-tryon] v30 Category: vtonCompatible=${catConfig.vtonCompatible}, compositeCategory="${compositeCategory}", isSaree=${isSaree}`)
  ...
  // ═══════════════════════════════════════════════════════════════════
  //  v30: Gemini Nano Banana is PRIMARY when a valid key is set.
  ...
  if (hasGeminiKey && !strategiesAttempted.includes('gemini') && Date.now() < totalDeadline - 20_000) {
    ...
    console.log('[virtual-tryon] v30 PRIMARY: Google Gemini Nano Banana (valid key set) — handles ALL categories')
  ...
  // ═══════════════════════════════════════════════════════════════════
  //  v30: Category-specific PRIMARY strategies
  // ═══════════════════════════════════════════════════════════════════
  if (isNonGarment) {
    if (isSaree) {
      // ── SAREES ──
      // v30: Showcase Composite is PRIMARY for sarees on Vercel.
      ...
      // Pollinations generates a NEW person → "different person AND different
      // product" mismatch.
      // Showcase Composite ALWAYS shows the user's real face + real saree
      // side-by-side → no mismatch possible.
      console.log('[virtual-tryon] v30 SAREE: Using Showcase Composite as PRIMARY (avoids mannequin mismatch)')
      if (input.productImageBase64) {
        return await buildShowcaseResult()
      }
    } else {
      // ── JEWELRY / WATCHES / ACCESSORIES / FRAGRANCES ──
      // v30: Image Composite is PRIMARY (with NEW mannequin detection).
      ...
      if (input.productImageBase64 && !strategiesAttempted.includes('composite')) {
        ...
        console.log(`[virtual-tryon] v30 PRIMARY: Image Composite v3 (with mannequin detection) — category="${compositeCategory}"`)
      ...
      // Used when composite fails OR mannequin is detected.
    ...
    if (catConfig.vtonCompatible && !strategiesAttempted.includes('idm-vton') && Date.now() < totalDeadline - 25_000) {
      ...
      console.log('[virtual-tryon] v30 PRIMARY (garment): IDM-VTON HF Space')
    ...
    // v30: Skip Image Composite for garments — garment product images
    // typically show a model wearing the garment → composite would place
    // the model over the user's face → "different person" mismatch.
    // (The mannequin detection in image-composite.ts would catch this, but
    //  we skip it entirely to save time and avoid the mismatch.)

    // ── FALLBACK: Pollinations with SELFIE reference (only if time permits) ──
    // Pollinations may generate a new person, but for garments the selfie
    // reference helps preserve approximate identity. Used as a degraded
    // fallback before the ultimate showcase.
    if (Date.now() < totalDeadline - 15_000) {
      strategiesAttempted.push('pollinations')
      console.log('[virtual-tryon] v30 FALLBACK (garment): Pollinations (selfie-ref)')
