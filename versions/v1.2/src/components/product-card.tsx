...
export function ProductCard({ product }: { product: Product }) {
  ...
  const getProxiedImageUrl = (url: string): string => {
    if (url.startsWith('http://') || url.startsWith('https://')) {
      ...
      return `/api/image-proxy?url=${encodeURIComponent(url)}&platform=${product.platform || ''}&name=${encodeURIComponent(product.name)}`;
    }
    if (url.startsWith('//')) {
      return `/api/image-proxy?url=${encodeURIComponent('https:' + url)}&platform=${product.platform || ''}&name=${encodeURIComponent(product.name)}`;
    }
    return url;
  };
