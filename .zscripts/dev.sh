...
# No set -euo pipefail - we want the script to keep running even if the server crashes
...
# No cleanup trap - we want the server to survive and auto-restart
...
# Auto-restart loop: if the server dies, restart it
while true; do
  if ! kill -0 "$DEV_PID" 2>/dev/null; then
    echo "[$(date)] Next.js server died, restarting..." 
    bun run dev &
    DEV_PID=$!
    echo "[$(date)] Restarted with PID: $DEV_PID"
    wait_for_service "localhost" "3000" "Next.js dev server" 30
  fi
  sleep 2
done
